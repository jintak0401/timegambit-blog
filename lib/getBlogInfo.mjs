...
export async function getAllSeries() {
  ...
  for (const { series, lastmod, images } of allBlogs) {
    ...
    if (!seriesInfo[series]) {
      const [img] = await globby(`public/static/images/blog/${slug(series)}/cover.{webp,jpg,jpeg,png,gif}`) || images;
      seriesInfo[series] = { title: series, length: 0, lastmod, image: img.replace('public', ''), href: slug(series) };
    }
