...
`Object.defineProperty`의 3번째 인자인 descriptor에는 4개의 속성이 있습니다.
* `value`: 값. 기본값은 undefined
* `writable`: 값이 변경 가능한지 여부. 기본값은 false
* `enumerable`: 열거 가능한지 여부. 기본값은 false
* `configurable`: enumerable, configurable, get, set을 변경할 수 있는지 여부. 기본값은 false
...
```
위에서 `writable`, `enumerable`, `configurable`의 기본값은 false라고 했습니다. 이 기본값은 `Object.defineProperty`를 사용하여 `value`를 설정할 때만 적용됩니다. `obj.birth = 1996`과 같이 값을 설정할 때는 `writable`, `enumerable`, `configurable`의 기본값이 true로 설정됩니다.
...
```
`writable`이 false이면 `obj.key = value` 방식으로는 값을 변경할 수 없습니다. `writable`이 false일 때 `obj.key = value`로 값을 변경하려고 시도하면, **strict mode가 아닐 경우에는 에러가 발생하지 않고 값 할당만 되지 않으며, strict mode일 경우에는 동일한 값을 할당하려 해도 에러가 발생합니다.** 하지만 `Object.defineProperty`를 사용하여 `value`를 변경할 수는 있습니다.
...
`configurable`이 false이면 `delete`로 객체에서 해당 속성을 삭제할 수 없습니다.(strict mode일 경우 에러가 발생합니다.) 또한 `Object.defineProperty`를 사용하여 `enumerable`, `configurable`, `get`, `set`을 변경할 수 없습니다.
...
console.log(b.x); // 1
```
15번 라인의 `a.x = 1`을 통해 8번 라인의 `set(x)`가 호출됩니다. 이 때 x는 =의 오른쪽에 있던 값인 1이 들어가게 됩니다. 16번 라인의 `b.x`를 통해 5번 라인의 `get()`을 호출하게 됩니다. 이 함수의 return 값인 `value(= 1)`가 `b.x`가 되어 1을 출력합니다.
...
```
위와 같이 `this`를 이용해 객체의 새로운 속성으로 값을 저장할 수 있습니다. `get()`과 `set()` 안의 `this`는 객체를 가리키게 됩니다.
