---
title: '[React Query] cache와 stale 동작원리'
date: '2023-04-24'
lastmod: '2023-04-24'
tags: ['React Query','라이브러리 분석']
draft: false
series: 동작원리
summary: React Query의 stale과 cache의 동작원리를 분석한 포스트입니다. fresh 상태에서 stale로 어떻게 바뀌는지, cache가 어떻게 삭제되는지 등을 설명하였습니다.
...
<Alert type="warning">
  ...
  이번 포스트는 useQuery 동작원리 포스트 [1편](https://timegambit.com/blog/digging/react-query/01)과 [2편](https://timegambit.com/blog/digging/react-query/02)를 읽었다는 것을 전제로 작성하였습니다. 그래서 1편과 2편에서 설명한 내용들은 생략하였으니, 만약 잘 이해가 안가신다면 1편과 2편을 읽고 오시는 것을 추천드립니다.
</Alert>
